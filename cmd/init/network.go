...
func NetworkSetup() error {
	...
	// netlink.LinkSetMaster(eth0, lo)
	addr, err := netlink.ParseAddr("192.168.1.14/16")
	if err != nil {
		return fmt.Errorf("error parsing address: %v", err)
	}

	if err := netlink.AddrAdd(eth0, addr); err != nil {
		return fmt.Errorf("failed to add address to interface: %v", err)
	}
...
func WriteEtcHost(hosts []EtcHost) error {
	...
	entries := []EtcHost{
		...
		{IP: "::1", Host: "localhost localhost6"},
	}
